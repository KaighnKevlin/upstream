...
- Art pass: terrain back to a 128 px period (64 slices) so the dirt no longer visibly repeats. Atlases split into terrain_atlas_0..5.png (source = host block*2 + mask>>3, 512 rows / 8192 px each: GL would not load a 16384 px texture, "unloadable"). Back wall is now tinted with a layer modulate instead of a per-pixel GDScript darkening pass at startup.
- Showcase: FACTORY (far west, ground flattened from tile 18): iron tapper (36 deg, 610) lobs through a laser at (526,-30) so the ingot drops into an assembler (600) making iron shot; spout -> belt (640..855) -> the upstream lift. Copper tapper (60 deg, 655, a flat lob kept under the laser) feeds a gravity wheel (520) that powers the assembler and belt. Test: wheel 0.3-0.9 power, 21 shot in 12 s, lift fills from the belt. Other chains unchanged.
- Content pass: TESLA COIL (U, Defence tab; scenes/tesla.gd, tools/art/gen_tesla.py 4 frames 40x70). Second turret type, runs on ingots (copper +4 charge, iron +6, max 30) through a side hopper; zaps the nearest enemy within 230 px, chains to 2 more within 95 px for 4/2/1 dmg; cooldown 1 s / power rate (unpowered 2.9 s); range and rate scale with Rifled barrels. Jagged bolt + glow light. Test: 2 ingots -> charge 10; magpie downed; chained hits 4/2/1 on a soldier cluster.
- Content pass: FLAME TURRET (I, Defence tab; scenes/flamer.gd, tools/art/gen_flamer.py furnace 3 frames 44x48 + nozzle). Burns ore as fuel (copper 3 s, iron 5 s, max 30) fed through its funnel; swivels a nozzle to the nearest enemy within 125 px and sprays a 20-degree particle cone with a flickering light; enemies caught keep burning 2 s (1 dmg / 0.4 s, tracked by the turret); ore that spends 0.15 s in the flame melts into an ingot mid-flight. Test: 2 ore -> 6 s fuel; a pack of 4 scuttlers burned down; a lobbed ore came out an ingot. Completes the three defence choices: cannon (ore as shot), tesla (ingots), flamer (ore as fuel).
