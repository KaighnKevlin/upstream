...
caster_death.png (6 frames): overload flash, thruster cuts out and it drops,
tipping over, hits the ground, rocks, lies dark.
"""
import math, sys
...
def build(i, charge=0.0, arcs=False, fire=False, dead=False, tilt=0.0, drop=0.0):
    ph = i / N * 2 * math.pi
    bob = 1.5 * math.sin(ph) if not (dead or drop) else 0.0
    y0 = -18 - bob          # body centre
    fig = Figure()
    if tilt or drop:
        fig.transform(tilt, (0, y0), (0, drop))
    glow = DARK if dead else GLOW
    if not (tilt or drop):
    ...
    for k, (bx, tx, z) in enumerate(((-4.5, -6.0, 3), (2.0, 3.0, 8))):
        ...
        fig.sphere(top, tip_r, glow, z=z + 0.2, emissive=not dead)
        tips.append(top)
    ...
    fig.sphere((4.8, y0), 3.7 + (1.0 if fire else 0) - (1.2 if dead else 0), glow, z=7, emissive=not dead)  # core eye
...
def death():
    return [build(0, 1.2, arcs=True, fire=True),              # overload
            build(1, dead=True, tilt=12, drop=4),
            build(2, dead=True, tilt=34, drop=9),
            build(3, dead=True, tilt=62, drop=10.5),          # impact
            build(4, dead=True, tilt=74, drop=9.5),           # rocks
            build(5, dead=True, tilt=80, drop=10.5)]
...
def main():
    ...
    dead = death()
    for name, frames in (('caster_hover', hover), ('caster_attack', attack), ('caster_death', dead)):
    ...
    if len(sys.argv) > 1:
        big = side_by_side(dead, 7)
        write_png(sys.argv[1] + '/caster_death_preview.png', len(big[0]), len(big), big)
        big = side_by_side(hover + attack, 7)
        write_png(sys.argv[1] + '/caster_preview.png', len(big[0]), len(big), big)
        pass  # write_gif(sys.argv[1] + '/caster.gif', hover + hover + attack + hover, [9] * (N * 3 + 6), 8)
